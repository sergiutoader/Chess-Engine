public class OpponentMove {
	...
	public static void opponentMove(Game game, String command) {
		...
		int prevRow = game.getRow(command.substring(0, 2));
		int prevColumn = game.getColumn(command.substring(0, 2));
		int currRow = game.getRow(command.substring(2));
		int currColumn = game.getColumn(command.substring(2));
	...
	// care se poate muta
	private static void engineEnPassant(Game game, int prevRow, int prevColumn, int currRow, int currColumn) {
		...
		if (game.side == false && prevRow == 1 && currRow == 3) {
			if (currColumn > 0 && game.grid[currRow][currColumn - 1] instanceof Pawn
				...
				game.grid[currRow][currColumn - 1].enPassant = game.getPosition(currRow - 1, currColumn);
			}
	...
	// grid-ul
	private static void opponentEnPassant(Game game, int prevRow, int prevColumn, int currRow, int currColumn) {
				...
				if (prevRow == 4 && currRow == 5) {
					if (prevColumn == currColumn + 1) {
						if ((game.grid[currRow - 1][currColumn] instanceof Pawn)
							...
							game.grid[currRow - 1][currColumn] = new Empty(game.getPosition(currRow - 1, currColumn), false,
									game);
						}
	...
	// grid-ul
	private static void opponentCastling(Game game, int prevRow, int prevColumn, int currRow, int currColumn) {
		if (game.grid[currRow][currColumn] instanceof King) {
			...
			if (currRow == prevRow && currColumn == prevColumn + 2) {
				...
				game.grid[currRow][currColumn - 1].position = game.getPosition(currRow, currColumn - 1);

				game.grid[currRow][7] = new Empty(game.getPosition(currRow, 7), false, game);
			}
			// rocada de 3
				...
				game.grid[currRow][0] = new Empty(game.getPosition(currRow, 0), false, game);
			}
		}
